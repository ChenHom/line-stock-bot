...
### Session 2025-12-02
- Q: 當使用者輸入的股票名稱模糊比對到多筆結果時，系統應如何回應？ → A: 系統應列出相似結果列表供使用者選擇確認，而非僅提示錯誤訊息。
...
2. **Given** 使用者已加入 LINE Bot 為好友，**When** 使用者發送「股價 致茂」(使用股票名稱)，**Then** 系統使用模糊比對識別股票，若找到多筆相似結果，則回應包含這些相似股票的列表供使用者選擇確認
...
- **模糊名稱比對結果不明確**: 使用者輸入「股價 致茂」等名稱，導致找到多筆相似結果時，系統應回應包含這些相似股票的列表供使用者選擇確認
...
- **FR-001**: 系統 MUST 支援「股價 <股票代號或名稱>」指令，回應該股票的即時行情資訊。當使用者輸入股票名稱時，系統 MUST 使用模糊比對 (fuzzy matching)；若找到多筆相似結果，則回應包含這些相似股票的列表供使用者選擇確認；若完全無相似結果，則提示「查無此股票代號，請確認後再試」
